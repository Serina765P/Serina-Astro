...
## 十五、S19 取消记录（P2-8 字体自托管 —— 用户否决）

**2026-09-13 用户决定：取消 S19。** 站点字体维持 ZeoSeven CDN 加载不变。

新政策（长期有效，跨项目）：**字体一律走 CDN；只有字形总数 ≤10 个的特殊场景才允许下载字形文件；
严禁字体自托管，严禁把字体二进制文件提交进仓库。**

- S19 已试做并完整回滚：vendor-fonts.mjs 重写版、BaseLayout 本地接线、996 个 woff2、
  两份本地 CSS、font-preload.json、_headers 全部移除（构建产物 CSS 哈希与 S18 状态逐字节一致）。
- `scripts/vendor-fonts.mjs`（江城圆体遗留脚本）已删除 —— 其唯一用途就是自托管，按新政策清掉。
  原计划第四节第 3 条「不删除、S19 重写复用」的决议随之作废。
- 试做期间的两条技术事实留档，供日后参考：
  1. 该缺陷「构建丢 CSS」失败时 HTML 不会引用 CSS（资产没产出，`<link>` 跟着消失），两症状同源。
  2. 字体二进制内容一致时，CDN 加载与本地加载的渲染存在**亚像素级栅格差异**（每行文字的
     pixel-diff 都会报差异，但肉眼不可辨）—— 字节哈希不能作为字体相关改动的判定器。
- 字体相关的残余改动为零：README 字体节、global.css 字体注释均维持 CDN 表述。
